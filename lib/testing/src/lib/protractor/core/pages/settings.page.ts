...
// @deprecated This is a DemoShell related class and will be removed with the future releases
export class SettingsPage {
    ...
    private async clickSsoRadioButton() {
    ...
    private async setLogoutUrl(logoutUrl) {
    ...
    private async setClientId(clientId: string = browser.params.testConfig.appConfig.oauth2.clientId) {
        await BrowserActions.clearSendKeys(this.clientIdText, clientId);
    }

    private async setContentServicesURL(contentServiceURL) {
        await BrowserActions.clearSendKeys(this.ecmText, contentServiceURL);
    }

    private async setAuthHost(authHostURL) {
        await BrowserActions.clearSendKeys(this.authHostText, authHostURL);
    }

    private async setIdentityHost(identityHost) {
    ...
    private async setSilentLogin(enableToggle) {
    ...
    private async setImplicitFlow(enableToggle) {
